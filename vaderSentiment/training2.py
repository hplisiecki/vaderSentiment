...
vader = vader[440:]
columns = ['Word', 'negative', 'positive', 'neutral']
vader.columns = columns

common = [x for x in list(vader.Word.values) if x in list(bradley.Description.values)]
df_test = bradley[~bradley.Description.isin(common)]
vader['norm_valence'] = (vader['negative'].values) / 8 + 0.5

df_train = vader.sample(frac=0.9, random_state=42)
df_val = vader.drop(df_train.index)
...
# save
# df_train.to_csv('vader_train.csv', index=False)
# df_test.to_csv('vader_test.csv', index=False)
# df_val.to_csv('vader_val.csv', index=False)

# append AoA and Concreteness scores to the previously generated train/val/test sets
df_train = pd.read_csv('vader_train.csv')


df_test = pd.read_csv('vader_test.csv')



df_val = pd.read_csv('vader_val.csv')
...
wandb.init(project="vader", entity="hubertp")
...
for epoch_num in range(epochs):
    ...
    with torch.no_grad():
        ...
        if best_corr_total < total_corr / len(val_dataloader):
            ...
            torch.save(model.state_dict(), 'models/vader.pth')
...
model.load_state_dict(torch.load('models/vader.pth'))
